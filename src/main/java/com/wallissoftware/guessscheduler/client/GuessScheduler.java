package com.wallissoftware.guessscheduler.client;
...
public class GuessScheduler extends SchedulerImpl {
    /**
     * Metadata bag for command objects. It's a JSO so that a lightweight
     * JsArray can be used instead of a Collections type.
     */
        ...
        public boolean executeRepeating(Duration duration, double goalDuration) {
            ...
            if (repeat) {
                ...
                if (repeat) {
                    ...
                    while (repeat && elapsedMilli < goalDuration) {
                        repeat = executeRepeating();
                        elapsedMilli = duration.elapsedMillis();
                        runCount += 1;
                    }
